...
```bash
pi install npm:@misunders2d/pi-goal@1.0.19
```

Try without installing:

```bash
pi -e npm:@misunders2d/pi-goal@1.0.19
```

Pinned GitHub release:

```bash
pi install git:github.com/misunders2d/pi-goal@v1.0.19
```
...
`/goal <outcome>` activates setup mode in the current conversation. The same main agent sees the normal session history, resolves references from prior discussion, debates conflicts, and asks only materially necessary follow-up questions. Clarification has no artificial round limit. Before approval, operational tools—including local file, attachment/upload, and screenshot inspection—are hard-blocked; the agent may only discuss the goal, inspect sanitized goal status, or submit a complete typed contract. Questions and answers remain visible in the ordinary conversation transcript. Once clear, the agent submits observable done conditions, ordered phases, mechanical verification checks, constraints, non-goals, and all foreseeable scoped authorities needed for fire-and-forget completion. Contract repair is runtime-bounded: two equivalent rejected submissions or three distinct rejected submissions pause setup and abort the active turn. Only a material message such as `Correction: <changed contract detail>` resets that budget; complaints, acknowledgements, questions, attachments/uploads, and screenshot paths do not. Run bare `/goal` to review the contract, refine it back in the same conversation, cancel it, or approve once. `/goal cancel` immediately cancels any active setup or goal. Bare `/goal` otherwise opens progress and controls.
...
- Filesystem boundaries use canonical resolved paths, reject symlink traversal outside the selected approved root, and recheck sensitive resolved targets. Additional roots must use canonical non-symlink paths and be explicitly setup-approved. A planned missing root is bound through its nearest existing canonical ancestor; later file or symlink substitution interrupts the goal fail-closed.
